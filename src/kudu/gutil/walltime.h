...
namespace walltime_internal {
  ...
  static GoogleOnceType timebase_info_once = GOOGLE_ONCE_INIT;
  static mach_timebase_info_data_t timebase_info;

  static void InitializeTimebaseInfo() {
  ...
  inline MicrosecondsInt64 GetMonoTimeMicros() {
    ...
    // mach_absolute_time returns ticks, which need to be scalled by the
    // timebase info to get nanoseconds. Dividing by 1000 scales to micros.
    return time * timebase_info.numer / (timebase_info.denom * 1000);
  }
  ...
  inline MicrosecondsInt64 GetThreadCpuTimeMicros() {
    ...
    return thread_info_data.user_time.seconds * 1000 + thread_info_data.user_time.microseconds;
  }
